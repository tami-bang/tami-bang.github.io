...
export const projects: Project[] = [
  ...
  },
  {
    ...
    subtitle:
      "Photoshop 이미지 제작과 Cafe24 스킨 수정을 중심으로 23장 분량의 요청을 이틀 만에 반영한 실무 의뢰",
    domain: "Cafe24 Storefront / Visual Content Maintenance",
    description:
      "경기도일자리재단 보내드림 활동가로 참여해 생활용품 자사몰 ‘생활을 담다, 생담’의 카테고리 이미지와 배너를 Photoshop으로 제작하고 Cafe24 스킨에 반영했습니다. 기존 HTML 영역은 불필요한 블록을 주석 처리하고 이미지 파일명과 링크 경로를 수정하는 방식으로 정리했습니다.",
    ...
    repeatedProblem:
      "기존 자사몰의 오래된 카테고리 이미지와 잘못된 명칭·링크·노출 영역을 요청서에 맞게 교체해야 하는 문제",
    structuredFlow:
      "23장 요청서 분석 → Photoshop 이미지 제작 → Cafe24 파일 업로드·스킨 수정 → PC·모바일 확인 → 결과 보고",
    automationPoint:
      "이미지 제작, 파일명 연결, 소스 주석 처리, 링크 수정 항목을 구분해 누락 없이 반영",
    resultSummary:
      "21개 항목 수정 완료, 1개 브라우저 제약 분석 및 대안 안내, 이틀 내 의뢰 완료",
    problem:
      "생담 자사몰에는 판매 품목과 맞지 않는 카테고리 이미지, 잘못된 명칭과 배너, 누락된 링크, 불필요하게 노출되는 스킨 영역이 남아 있었습니다. 23개 슬라이드에 표시된 화면별 요청을 확인해 새로운 이미지를 제작하고, Cafe24의 기존 스킨 구조를 크게 바꾸지 않으면서 PC와 모바일에 반영해야 했습니다.",
    solution:
      "작업 시간의 대부분은 Photoshop에서 카테고리별 이미지를 새로 만들고 배너를 수정하는 데 사용했습니다. 완성한 이미지는 요청된 파일명으로 정리해 Cafe24에 다시 업로드하고, 스킨 소스에서 이미지 경로와 문구를 교체했습니다. BEST, 추천메일, 상품조르기, SNS 홍보처럼 제거할 영역은 기존 코드를 삭제하지 않고 주석 처리했으며 이용안내와 카카오톡 링크를 올바른 경로로 수정했습니다. 기존 검색·게시판·모바일 기능은 Cafe24 스킨을 새로 개발한 것이 아니라 요청에 맞게 소스를 정리하고 정상 작동 여부를 확인했습니다.",
    architecture: [
    ...
    results: [
      ...
      "요청 분석부터 이미지 제작, 스킨 반영, 검증, 완료 보고까지 이틀 안에 마무리",
