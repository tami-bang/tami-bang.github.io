...
export const projects: Project[] = [
  ...
  },
  {
    ...
    title: "생담 자사몰 유지보수",
    subtitle:
      "23장 분량의 개선 요청서를 분석하고 구현·검증·결과 보고까지 이틀 만에 완료한 실무 의뢰",
    domain: "Website Maintenance / Client Work",
    description:
      "경기도일자리재단 보내드림 활동가로 참여해 생활용품 자사몰 ‘생활을 담다, 생담’의 카테고리, 이미지, 배너, 링크, 검색, 상품 상세, 게시판 영역을 개선했습니다. 23장 분량의 요청서에 정리된 22개 작업 항목 중 21건을 수정하고, 브라우저 정책상 구현할 수 없는 1건은 기술적 이유와 현실적인 대안을 문서화했습니다.",
    ...
    structuredFlow:
      "23장 요청서 분석 → 유형별 분류 → 수정·기능 구현 → PC·모바일 검증 → 결과 보고",
    automationPoint:
      "단순 수정과 기능·호환성 검토를 분리해 짧은 일정에서도 누락 없이 처리할 수 있는 점검 흐름 구성",
    resultSummary:
      "21개 항목 수정 완료, 1개 브라우저 제약 분석 및 대안 안내, 이틀 내 의뢰 완료",
    problem:
      "생담 자사몰에는 잘못된 카테고리명과 상품 이미지, 누락된 링크, 깨진 고객센터 정렬, 소스 코드가 노출되는 게시판 등 서로 다른 유형의 문제가 함께 존재했습니다. 전달받은 자료는 23개 슬라이드로 구성되어 있어 각 요청의 수정 범위와 완료 기준을 빠르게 파악하고, PC와 모바일 화면에 일관되게 반영해야 했습니다.",
    solution:
      "슬라이드별 요청을 콘텐츠, 이미지·배너, 화면 구성, 링크·기능, 검색 경험, 오류 수정으로 분류한 뒤 관련 화면을 묶어 작업했습니다. 검색 결과 페이지에는 결과 내 재검색과 초기화 기능을 구현했고, 상품문의 구조를 수정해 고객센터 레이아웃을 정상화했습니다. 게시판 공통 소스 노출 문제를 해결한 뒤 갤러리, 1:1 상담, 상품후기, 운영일지, 자유게시판과 게시판 메인까지 회귀 검증했습니다. 모바일은 기존 반응형 동작이 대부분 정상인 상태였기 때문에 변경된 상품별 카테고리 이미지와 명칭을 동일하게 반영하고 주요 동작을 확인했습니다. 사이트가 즐겨찾기를 직접 등록해 달라는 요청은 현재 브라우저 보안 정책상 구현할 수 없다고 판단해 기존 상태를 유지하고 Ctrl+D 사용을 대안으로 안내했습니다.",
    architecture: [
      ...
      "상품 상세 정보 구조와 고객센터·상품문의 레이아웃 정상화",
      "게시판 공통 소스 코드 노출 문제 수정",
      "갤러리, 1:1 상담, 상품후기, 운영일지, 자유게시판과 게시판 메인 회귀 검증",
      "모바일 상품별 카테고리 이미지·명칭 반영 및 주요 동작 확인",
      "PC 화면의 검색·초기화와 상담 이동 흐름 검증",
    ...
    highlights: [
      ...
      "검색 결과 안에서 키워드를 다시 적용하고 원래 목록으로 초기화할 수 있는 재검색 기능 구현",
      "‘생수병’ 검색 결과 98개를 ‘1.8’ 조건으로 14개까지 좁히고 초기화 시 98개로 복원되는 흐름 검증",
      "상품 상세 페이지의 중복 상품명·추천메일·상품조르기·SNS 홍보를 제거하고 단골회원가를 판매가 아래로 재배치",
      "비회원 대상 네이버페이 간편구매 영역의 노출 상태 확인",
      "깨진 상품문의 구조를 수정해 고객센터 검색·Q&A·FAQ 영역의 정렬 정상화",
      "공지사항과 공통 게시판 템플릿에서 내부 소스 코드가 사용자에게 노출되는 오류 해결",
      "갤러리 상세·답글, 1:1 상담 답변·상세, 상품후기, 운영일지, 자유게시판과 게시판 메인까지 회귀 확인",
      "카카오톡 버튼을 일반 홈페이지가 아닌 생담 채널의 직접 상담 URL로 연결",
      "기존 모바일 동작을 유지하면서 변경된 상품별 카테고리 이미지와 명칭을 모바일 화면에도 동일하게 반영",
    ...
    results: [
      ...
      "재검색 적용·초기화, 이용안내 링크, 카카오톡 직접 상담 등 핵심 사용자 흐름 정상 동작 확인",
      "여러 유형의 게시판 상세·답글·목록·메인에서 소스 코드가 노출되지 않는지 회귀 검증",
      "모바일 상품별 카테고리 이미지 변경과 기존 반응형 동작 유지 여부 확인",
